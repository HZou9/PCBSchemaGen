...
from subcircuit_mapping import format_subcircuit_reference, is_complex_task as is_subcircuit_task
...
def run_task_flow(task, input_nodes, output_nodes, task_id, it, flog, input_voltage="", output_voltage=""):
    ...
    if args.prompt_mode == "simple":
        ...
        if complex_task_validator.is_complex_task(task_id):
            prompt_file = "prompt_template_complex_pcb_retrieval.md"
        else:
            prompt_file = "prompt_template.md"
    ...
    # Subcircuit retrieval for complex tasks (P17-P23)
    if is_subcircuit_task(task_id):
        sample_dir = os.path.join(BASE_DIR, "sample design")
        subcircuit_ref = format_subcircuit_reference(task_id, sample_dir)
        prompt = prompt.replace("[SUBCIRCUIT_REFERENCE]", subcircuit_ref)
        print(f"[Subcircuit Retrieval] Injected reference code from: p7, p9, p15")
    else:
        prompt = prompt.replace("[SUBCIRCUIT_REFERENCE]", "")
    ...
    for index, row in df.iterrows():
        ...
        try:
            run_task_flow(
                ...
                it=0,
                flog=flog,
                input_voltage=row.get('InputVoltage', ''),
                output_voltage=row.get('OutputVoltage', '')
            )
        finally:
            flog.close()
