...
parser.add_argument(
    ...
    type=str,
    default="auto",
    choices=["auto", "simple", "no_icl", "no_cot"],
    help="Prompt template selection: auto (default), simple (always simple), no_icl (no in-context learning), no_cot (no chain-of-thought).",
)
...
def run_task_flow(task, input_nodes, output_nodes, task_id, it, flog, input_voltage="", output_voltage=""):
    ...
    # Prompt mode selects the template file:
    #   "auto"   → P1-16 simple, P17-23 complex (default)
    #   "simple" → always simple prompt
    #   "no_icl" → prompt without in-context learning example
    #   "no_cot" → prompt without chain-of-thought reasoning
    is_complex = complex_task_validator.is_complex_task(task_id)
    if args.prompt_mode == "simple":
        prompt_file = "prompt_template.md"
    elif args.prompt_mode == "no_icl":
        if is_complex:
            prompt_file = "prompt_template_complex_pcb_no_icl.md"
        else:
            prompt_file = "prompt_template_no_icl.md"
    elif args.prompt_mode == "no_cot":
        if is_complex:
            prompt_file = "prompt_template_complex_pcb_no_cot.md"
        else:
            prompt_file = "prompt_template_no_cot.md"
    else:  # auto mode
        if is_complex:
    ...
    prompt = prompt.replace("[INPUT_VOLTAGE]", str(input_voltage))
    prompt = prompt.replace("[OUTPUT_VOLTAGE]", str(output_voltage))
    ...
    for index, row in df.iterrows():
        ...
        try:
            run_task_flow(
                ...
                it=0,
                flog=flog,
                input_voltage=str(row.get('InputVoltage', '')),
                output_voltage=str(row.get('OutputVoltage', '')),
            )
        finally:
            flog.close()
